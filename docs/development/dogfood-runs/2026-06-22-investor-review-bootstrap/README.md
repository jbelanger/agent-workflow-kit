...
| `#5 Break down the first walking skeleton into prepared task candidates` | Task | `breakdown-issue` | Blocked on artifact/ADR/spec review; does not route to implementation. |
| `#6 Prepare implementation brief for the first approved skeleton task` | Task | `prepare-implementation` | Blocked on breakdown; does not route to implementation. |
...
`review-artifact` on issue `#2`, followed by `#3` and `#4`, then `breakdown-issue` on `#5`.
